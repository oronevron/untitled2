...
#         #my_img2 = tf.image.resize_image_with_crop_or_pad(value, 40,40)
#         my_img_re = tf.image.resize_images(my_img, [40,40])
#         my_img_re = tf.image.random_flip_up_down(my_img_re, seed=None)
...
x2 = tf.placeholder(tf.float32, [None, 40,40,1 ])
...
W_conv3 = globalFunctions.weight_variable([filter_size, filter_size, n_filters_1, n_filters_2])
b_conv3 = globalFunctions.bias_variable([n_filters_2])

# layers
h_conv1 = tf.nn.relu(globalFunctions.conv2d(x_tensor2, W_conv1) + b_conv1)
...
for epoch_i in range(n_epochs):
    ...
    # Shuffle data.
    train_image, train_labal = globalFunctions.Shuffle(train_image, train_labal)

    prediction = tf.argmax(y_pred, 1)
    labels = prediction.eval(feed_dict={x2: validation_image, y: validation_labal, keep_prob: 1.0}, session=sess)

   # confusion_matrix1(one_hot_to_label_array(validation_labal), predictions[0])
    globalFunctions.confusion_matrix1(globalFunctions.one_hot_to_label_array(validation_labal), labels)
