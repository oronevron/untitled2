...
# with batchh normalization
n_fc = 1200
W_fc1 = globalFunctions.weight_variable([5 * 5 * n_filters_3, n_fc])
b_fc1 = globalFunctions.bias_variable([n_fc])
h_fc1 = tf.nn.relu(globalFunctions.batch_norm(tf.matmul(h_conv3_flat, W_fc1) + b_fc1,
                                              1200,
                                              phase_train))
...
h_fc2 = tf.nn.relu(globalFunctions.batch_norm(tf.matmul(h_fc1_drop, W_fc2) + b_fc2,
                                              800,
                                              phase_train))
...
for epoch_i in range(n_epochs):
    ...
    if epoch_i % 8 == 0:
        saver.save(sess, "FinalDataset/Model/Model6/Model")
