...
train_image,train_label,validation_image,validation_label,test_image,test_label = \
    globalFunctions.load_all_image(0, 'TF', 40, True, force_load=False)
...
labels = prediction.eval(feed_dict={x: test_image, y: test_label, keep_prob: 1.0}, session=sess)
globalFunctions.confusion_matrix1(globalFunctions.one_hot_to_label_array(test_label), labels, "test_cm")
globalFunctions.accuracy_per_epoch_validation(l_loss, "accuracy_per_epoch_validation")
globalFunctions.accuracy_per_epoch_train_and_validation(l_loss_train, l_loss, "accuracy_per_epoch_train_and_validation" )